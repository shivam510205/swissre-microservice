...
def summarize_medical_data(request: SummarizationRequest):
    ...
    try:
        ...
        logger.info(f"Summarization completed: {summary_id} for patient {request.patient_id}")
        return response
...
async def summarize_file(file: UploadFile = File(...), patient_id: Optional[str] = None):
